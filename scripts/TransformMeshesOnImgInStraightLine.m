function [cell_transformed_vertices, cell_transformed_faces] = TransformMeshesOnImgInStraightLine(cell_vertices, cell_faces, bg_im)
    %% transformation
    ...
    for i = 1:size(cell_vertices,1)
        ...
        if(i == 1)
            ...
            pt_size = size(cell_transformed_vertices{i-1, 1}, 1);
            rnd_pt_from_previous_mesh = cell_transformed_vertices{i-1, 1}(randi([1, pt_size]), :);
            fish_im_centroid(1) = rnd_pt_from_previous_mesh(:,1);
            fish_im_centroid(2) = rnd_pt_from_previous_mesh(:,2);
        end
        
        fish_im_centroid(3) = temp_z + abs(max(transformed_vertices(:,3)) - min(transformed_vertices(:, 3)));
        % translates if out of bounds
        ...
        translated_to_im_center_vertices = (fish_im_centroid - origin) + transformed_vertices;
            ...
            pt_size = size(transformed_vertices, 1);
            rnd_pt_from_current_mesh = transformed_vertices(randi([1, pt_size]), :);
            rnd_pt_from_current_mesh(:,3) = 0;
            translated_to_im_center_vertices = (fish_im_centroid - rnd_pt_from_current_mesh) + transformed_vertices;
        end
